...
// Universal share helper used by all sports
function _shareToSquad(text, title) {
  const shareTitle = title || 'Arena — Squad Scorebook';
  if (navigator.share) {
...
function _fallbackCopy(text) {
  navigator.clipboard.writeText(text)
    .then(() => toast('Copied! Paste into WhatsApp 📋'))
    .catch(() => toast('Copy manually: ' + text.slice(0, 40) + '…'));
}
...
// Share result for any active sport
function shareCurrentResult() {
  const sp = S.sport;
  let msg = '';
  const tag = '\n\n🏆 Tracked on Arena — Your Squad\'s Scorebook';
  
  if (sp === 'cricket') {
    const ch = S.cricket;
    const i1 = ch.innings[0], i2 = ch.innings[1];
    const winner = ch.winner || 'Match in progress';
    msg = `🏏 Cricket Match\n${ch.teamAName} vs ${ch.teamBName}\n` +
          `${ch.teamAName}: ${i1.runs}/${i1.wickets} (${i1.balls} balls)\n` +
          `${ch.teamBName}: ${i2.runs}/${i2.wickets} (${i2.balls} balls)\n` +
          `Result: ${winner}${tag}`;
  } else if (sp === 'football') {
    const fb = S.football;
    msg = `⚽ Football Match\n${fb.teamA} ${fb.scoreA} — ${fb.scoreB} ${fb.teamB}\n` +
          `${fb.winner ? 'Winner: ' + fb.winner : 'Match in progress'}${tag}`;
  } else if (sp === 'badminton') {
    const bd = S.badminton;
    msg = `🏸 Badminton Match\n${bd.playerA} vs ${bd.playerB}\n` +
          `Sets: ${bd.setsA}-${bd.setsB}\n` +
          `${bd.winner ? 'Winner: ' + bd.winner : 'Match in progress'}${tag}`;
  } else if (sp === 'pickleball') {
    const pk = S.pickleball;
    msg = `🥒 Pickleball Match\n${pk.teamA} vs ${pk.teamB}\n` +
          `Score: ${pk.scoreA}-${pk.scoreB}\n` +
          `${pk.winner ? 'Winner: ' + pk.winner : 'Match in progress'}${tag}`;
  } else if (sp === 'chess') {
    const ch = S.chess;
    msg = `♟️ Chess Match\n${ch.playerWhite} vs ${ch.playerBlack}\n` +
          `${ch.matchOver ? 'Result: ' + (ch.winner || 'Draw') : 'Game in progress'}\n` +
          `Moves: ${(ch.moveLog||[]).length}${tag}`;
  }
  
  if (!msg) { toast('Nothing to share yet!'); return; }
  
  // Show share options bottom sheet
  _showShareSheet(msg);
}

function _showShareSheet(msg) {
  // Remove existing
  const ex = document.getElementById('share-sheet');
  if (ex) ex.remove();
  
  const sheet = document.createElement('div');
  sheet.id = 'share-sheet';
  sheet.innerHTML = `
    <div class="share-sheet-backdrop" onclick="document.getElementById('share-sheet').remove()"></div>
    <div class="share-sheet-panel">
      <div class="share-sheet-handle"></div>
      <div class="share-sheet-title">Share with your squad</div>
      <div class="share-sheet-preview">${msg.replace(/\n/g,'<br>')}</div>
      <div class="share-sheet-btns">
        <button class="share-btn share-btn-wa" onclick="shareWhatsApp(${JSON.stringify(msg)});document.getElementById('share-sheet').remove()">
          <i class="ti ti-brand-whatsapp"></i> WhatsApp
        </button>
        <button class="share-btn share-btn-copy" onclick="_fallbackCopy(${JSON.stringify(msg)});document.getElementById('share-sheet').remove()">
          <i class="ti ti-copy"></i> Copy Text
        </button>
        ${navigator.share ? `<button class="share-btn share-btn-native" onclick="_shareToSquad(${JSON.stringify(msg)});document.getElementById('share-sheet').remove()">
          <i class="ti ti-share"></i> More
        </button>` : ''}
      </div>
    </div>`;
  document.body.appendChild(sheet);
  requestAnimationFrame(() => sheet.querySelector('.share-sheet-panel').style.transform = 'translateY(0)');
}
